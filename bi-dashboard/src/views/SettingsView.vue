  ...
  <div class="h-screen bg-slate-50 flex text-slate-800 font-sans overflow-hidden">
    ...
    <main class="flex-1 flex flex-col h-screen overflow-hidden bg-[#f4f7fb] relative">
      ...
      <div class="flex-1 overflow-y-auto p-4 sm:p-8 custom-scrollbar">
        ...
        <div class="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-2 bg-white p-1 rounded-2xl border border-slate-200 shadow-sm overflow-x-auto max-w-full">
                ...
                <button 
                  ...
                  :class="activeTab === 'integrations' ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-400 hover:bg-slate-50'"
                  class="px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2"
                >
                  <Zap class="w-3 h-3" /> Integrations
                </button>
                <button 
        ...
        <!-- INTEGRATIONS TAB -->
        <div v-if="activeTab === 'integrations'" class="max-w-4xl space-y-8 mb-8 animate-[slideDown_0.3s_ease-out]">
            <!-- Email (SMTP) Configuration -->
            <div class="bg-white rounded-3xl border border-slate-200 shadow-sm p-8 group overflow-hidden">
                <h3 class="font-black text-slate-900 uppercase tracking-widest text-[11px] mb-8 border-b border-slate-50 pb-4 flex items-center gap-2">
                    <Mail class="w-4 h-4 text-primary" /> Multi-Tenant Email Gateway (SMTP)
                </h3>
                <div class="space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest pl-1">Server Host</label>
                            <input v-model="emailSettings.smtp_host" type="text" placeholder="smtp.gmail.com" class="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 text-sm font-bold focus:ring-2 focus:ring-primary/20 outline-none transition-all">
                        </div>
                        <div class="space-y-2">
                            <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest pl-1">Port</label>
                            <input v-model="emailSettings.smtp_port" type="number" placeholder="587" class="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 text-sm font-bold focus:ring-2 focus:ring-primary/20 outline-none transition-all">
                        </div>
                    </div>
                    
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest pl-1">Username / Account</label>
                            <input v-model="emailSettings.smtp_user" type="text" placeholder="user@example.com" class="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 text-sm font-bold focus:ring-2 focus:ring-primary/20 outline-none transition-all">
                        </div>
                        <div class="space-y-2 relative">
                            <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest pl-1">App Password / Key</label>
                            <input :type="showSmtpPass ? 'text' : 'password'" v-model="emailSettings.smtp_pass" class="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 text-sm font-bold focus:ring-2 focus:ring-primary/20 outline-none transition-all pr-12">
                            <button @click="showSmtpPass = !showSmtpPass" class="absolute right-3 top-[34px] p-2 hover:bg-white rounded-lg transition-colors">
                                <Eye v-if="!showSmtpPass" class="w-4 h-4 text-slate-400" />
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-slate-50">
                        <div class="space-y-2">
                            <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest pl-1">Sender Name</label>
                            <input v-model="emailSettings.email_from_name" type="text" placeholder="DigyNex Operations" class="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 text-sm font-bold focus:ring-2 focus:ring-primary/20 outline-none transition-all">
                        </div>
                        <div class="space-y-2">
                            <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest pl-1">Sender Email</label>
                            <input v-model="emailSettings.email_from_address" type="text" placeholder="no-reply@domain.com" class="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 text-sm font-bold focus:ring-2 focus:ring-primary/20 outline-none transition-all">
                        </div>
                    </div>
                    ...
                    <div class="pt-4">
                        <button 
                            ...
                            class="w-full py-4 bg-slate-900 text-white rounded-2xl text-[11px] font-black uppercase tracking-widest shadow-2xl shadow-slate-900/20 active:scale-[0.98] transition-all flex items-center justify-center gap-3"
                        >
                            <Loader2 v-if="savingEmail" class="w-4 h-4 animate-spin text-primary" />
                            <Save v-else class="w-4 h-4 text-primary" />
                            Synchronize Gateway Settings
                        </button>
                    </div>
                </div>
            </div>

            <!-- Coming Soon: WhatsApp & Other APIs -->
            <div class="bg-slate-100/50 rounded-3xl border border-dashed border-slate-200 p-8 text-center grayscale opacity-80">
                <p class="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Advanced Matrix Nodes</p>
                <div class="flex justify-center gap-8 mt-4">
                    <div class="flex flex-col items-center gap-2">
                        <div class="p-4 bg-white rounded-2xl border border-slate-200 text-slate-300"><MessageSquare class="w-6 h-6"/></div>
                        <span class="text-[9px] font-black text-slate-400">WhatsApp API</span>
                    </div>
                    <div class="flex flex-col items-center gap-2">
                        <div class="p-4 bg-white rounded-2xl border border-slate-200 text-slate-300"><Layout class="w-6 h-6"/></div>
                        <span class="text-[9px] font-black text-slate-400">Facebook Meta</span>
                    </div>
                    <div class="flex flex-col items-center gap-2">
                        <div class="p-4 bg-white rounded-2xl border border-slate-200 text-slate-300"><Linkedin class="w-6 h-6"/></div>
                        <span class="text-[9px] font-black text-slate-400">LinkedIn Sales</span>
                    </div>
                </div>
            </div>
        </div>
...
import { Settings, X, Loader2, RotateCcw, ShieldAlert, ShieldCheck, UserCheck, Zap, Send, Users, Search, Bell, Plus, Info, Mail, Save, Eye, EyeOff, MessageSquare, Layout, Linkedin } from 'lucide-vue-next'
...
const saveEmailSettings = async () => {
    ...
    try {
        const { data: org } = await supabase.from('organizations').select('id').eq('slug', 'digynex-hq').single()
        if (!org) throw new Error("Cloud Node Context missing.")

        const { error } = await supabase.from('organizations').update({
            smtp_host: emailSettings.value.smtp_host,
            smtp_port: emailSettings.value.smtp_port,
            smtp_user: emailSettings.value.smtp_user,
            smtp_pass: emailSettings.value.smtp_pass,
            email_from_name: emailSettings.value.email_from_name,
            email_from_address: emailSettings.value.email_from_address
        }).eq('id', org.id)

        if (error) throw error
        triggerToast("Email Gateway successfully synchronized with Enterprise Cloud.")
    } catch (err) {
        console.error(err)
        triggerToast(`Gateway Failure: ${err.message}`)
    } finally {
