...
# Target pattern (found in Applications, Matches, Profile)
target_pattern = """            <div class="p-0.5 bg-white/10 rounded-full">
               <img src="/digynex-icon.png" alt="DigyNex" class="h-8 w-auto opacity-50 contrast-125" />
            </div>"""

# Replace occurrences
new_content = content.replace(target_pattern, auth_gate_code)

with open(file_path, 'w', encoding='utf-8') as f:
    f.write(new_content)

print(f"Successfully updated {new_content.count(auth_gate_code)} auth headers.")
